...
describe("AppJsonRpcProvider", () => {
  ...
  it("constructor initializes with valid providers", () => {
    expect(
      () => new AppJsonRpcProvider(ChainId.DEGEN, mockProviders),
    ).not.toThrow();
  });

  it("constructor throws with empty providers array", () => {
    expect(() => new AppJsonRpcProvider(ChainId.DEGEN, [])).toThrow(
  ...
  it("constructor throws with network mismatch", () => {
    ...
    expect(() => new AppJsonRpcProvider(ChainId.DEGEN, mockProviders)).toThrow(
      "networks mismatch",
    );
  });
  ...
  it("constructor throws with invalid providers", () => {
    ...
    expect(
      () => new AppJsonRpcProvider(ChainId.DEGEN, [{} as JsonRpcProvider]),
  ...
  it("handles and instruments call", async () => {
    ...
    const provider = new AppJsonRpcProvider(ChainId.DEGEN, [mockProvider1]);
    ...
    expect(
      provider.providerEvaluations[0].performance.latency,
  ...
  it("should increment failureCount on provider failure", async () => {
    ...
    const provider = new AppJsonRpcProvider(ChainId.DEGEN, mockProviders);
    ...
    const [fastProvider, slowProvider] = AppJsonRpcProvider.sortProviders([
      {
        ...
        performance: { latency: 500, callCount: 1, failureCount: 0 },
      },
      {
        provider: {} as JsonRpcProvider,
        performance: { latency: 1, callCount: 1, failureCount: 0 },
      },
    ]);
  ...
  it("should sort failing providers after successful providers", async () => {
    ...
    const [provider, failingProvider] = AppJsonRpcProvider.sortProviders([
      {
        provider: {} as JsonRpcProvider,
        performance: { latency: 1, callCount: 1, failureCount: 1 },
      },
      {
        provider: {} as JsonRpcProvider,
        performance: { latency: 1, callCount: 1, failureCount: 0 },
