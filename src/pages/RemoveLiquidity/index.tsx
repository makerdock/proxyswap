...
export default function RemoveLiquidity() {
  ...
  async function onRemove() {
    ...
    const currencyBIsETH = currencyB.isNative || currencyB.equals(DEGEN)
    const oneCurrencyIsETH = currencyA.isNative || currencyA.equals(DEGEN) || currencyBIsETH
  ...
  const oneCurrencyIsETH =
    currencyA?.isNative || currencyB?.isNative || currencyA?.equals(DEGEN) || currencyB?.equals(DEGEN)
                  ...
                  <AutoColumn gap="10px">
                    ...
                    {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                      <RowBetween style={{ justifyContent: 'flex-end' }}>
                        {oneCurrencyIsETH ? (
                          <StyledInternalLink
                            to={`/remove/v2/${(currencyA?.isNative || currencyA?.equals(DEGEN)) &&
                              chainId &&
                              WRAPPED_NATIVE_CURRENCY[chainId]
                              ? WRAPPED_NATIVE_CURRENCY[chainId]?.address
                              : currencyIdA
                              }/${(currencyB?.isNative || currencyB?.equals(DEGEN)) &&
                                chainId &&
                                WRAPPED_NATIVE_CURRENCY[chainId]
                                ? WRAPPED_NATIVE_CURRENCY[chainId]?.address
                                : currencyIdB
