...
const BLOCK_EXPLORER_PREFIXES: { [chainId: number]: string } = {
  ...
  [SupportedChainId.DEGEN]: 'https://explorer.degen.tips/',
}
...
export function getExplorerLink(chainId: number, data: string, type: ExplorerDataType): string {
  ...
  // @TODO
  if (chainId === SupportedChainId.DEGEN) {
    switch (type) {
      case ExplorerDataType.TRANSACTION:
        return `https://explorer.degen.tips/tx/${data}`
      case ExplorerDataType.ADDRESS:
      case ExplorerDataType.TOKEN:
        return `https://explorer.degen.tips/address/${data}`
      case ExplorerDataType.BLOCK:
        return `https://explorer.degen.tips/block/${data}`
      default:
